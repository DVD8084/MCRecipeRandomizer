...
zip_file.writestr('pack.mcmeta', json.dumps({
    'pack': {
        'pack_format': 5,
        'description': 'Randomize everything! The ultimate experience!'
    }
}))
...
for i, (filename, recipe) in enumerate(recipe_data.items()):
    ...
    recipe = internals.scramble_input(recipe, inputs)
    ...
    # Due to the fact that Minecraft has so many items, let's include a bit more of them. :P
    recipe = internals.scramble_input(recipe, inputs)
    recipe = internals.scramble_output(recipe, outputs)
    recipe['group'] = internals.get_output(recipe)
    zip_file.writestr(os.path.join(data_folder, '_' + filename), json.dumps(recipe))
